...
module "databricks_provisioning" {
  ...
  keyvault_dns             = data.azurerm_key_vault.kv.properties.vaultUri
  ...
  depends_on = [
    module.databricks,
    module.storage,
    azurerm_role_assignment.identity_storage_access_dbx,
    azurerm_role_assignment.access_connector_keyvault_access
  ]
}
