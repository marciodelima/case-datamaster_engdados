...
resource "azurerm_role_assignment" "news_blob_access" {
  ...
  principal_id         = azurerm_function_app.news_producer.identity[0].principal_id
}

resource "azurerm_role_assignment" "ri_blob_access" {
  scope                = data.azurerm_storage_account.existing_storage.id
  role_definition_name = "Storage Blob Data Contributor"
  principal_id         = azurerm_function_app.ri_resumer.identity[0].principal_id
}
...
resource "azurerm_role_assignment" "ri_collector_blob_access" {
  scope                = data.azurerm_storage_account.existing_storage.id
  role_definition_name = "Storage Blob Data Contributor"
  principal_id         = azurerm_function_app.ri_collector.identity[0].principal_id
}

resource "azurerm_role_assignment" "finance_blob_access" {
  scope                = data.azurerm_storage_account.existing_storage.id
  role_definition_name = "Storage Blob Data Contributor"
  principal_id         = azurerm_function_app.finance_csv_ingestor.identity[0].principal_id
}

resource "azurerm_role_assignment" "postgres_blob_access" {
  scope                = data.azurerm_storage_account.existing_storage.id
  role_definition_name = "Storage Blob Data Contributor"
  principal_id         = azurerm_function_app.postgres_ingestor.identity[0].principal_id
}

resource "azurerm_role_assignment" "sentiment_blob_access" {
  scope                = data.azurerm_storage_account.existing_storage.id
  role_definition_name = "Storage Blob Data Contributor"
  principal_id         = azurerm_function_app.news_sentiment_analyzer.identity[0].principal_id
}
